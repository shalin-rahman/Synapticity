...
class MissionEngine:
    ...
    async def run(self, mission_id: str, objective: str = None) -> None:
        ...
        print(f"[START] Building project {mission_id} (Phase: {state.get('phase', 'INITIAL')})")
        ...
        try:
            ...
            if not state.get("code"):
                ...
                raw_code      = await self.coder.run(
                    state["specs"], context,
                    task="Writing the code", mission_id=mission_id,
    ...
    async def _finalize(self, path, state, mission_id, workspace: MissionWorkspace, state_mgr: MissionStateManager) -> None:
        ...
        print("[DOCS] Writing the project guide...")
        docs = await self.writer.run(
            f"Objective: {state['objective']}\nSpecs: {state['specs']}\nFinal Code: {state['code']}",
            task="Drafting the documentation", mission_id=mission_id
        )
        self._log.log(state, "writer", "Wrote the project docs.", docs)

        print("[CI] Setting up the launch script...")
        pipeline = await self.devops.run(
            f"Objective: {state['objective']}\nRepository Tech Stack Code:\n{state['code']}",
            task="Building the deployment script", mission_id=mission_id
        )
        self._log.log(state, "devops-engineer", "Created the deployment script.", pipeline)
